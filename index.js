...
function addGamesToPage(games) {
  games.forEach((game) => {
    ...
    gameCard.innerHTML = `<img class="game-img" src="${game.img}" alt="${game.name}"><h3>${game.name}</h3> <p>${game.description}</p><p>Backers: ${game.backers}</p>`;
...
const display = `A total of $${amountRaised.toLocaleString()} has been raised for ${
  GAMES_JSON.length
} games. Currently, ${
  numUnfundedGames <= 1
    ? numUnfundedGames + " game remains"
    : numUnfundedGames + " games remain"
} unfuned. We need your help to fund these amazing games!`;
